...
WebSQLStore.sync = Backbone.WebSQLsync = function (method, model, options) {
	...
	switch(method) {
		...
		case "create":	store.create(model,success,error);
			break;
		case "update":	store.find(model,findSuccess,error);
			break;
...
Backbone.getSyncMethod = function(model) {
	if(model.WebSQLStore || (model.collection && model.collection.WebSQLStore))
	{
